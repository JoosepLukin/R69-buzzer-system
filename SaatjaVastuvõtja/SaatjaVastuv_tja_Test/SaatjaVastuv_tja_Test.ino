...
void handleRS485Command(const String &line) {
    ...
    // ── ping ─────────────────────────────────────────────────────────────────
        ...
        // Reply to KontrollKast so it knows SaatjaVastuvõtja itself is alive
        {
            StaticJsonDocument<150> reply;
            reply["type"]    = "ping_reply";
            reply["mac"]     = WiFi.macAddress().c_str();
            reply["battery"] = 0.0;
            String out;
            serializeJson(reply, out);
            Serial.print("[PING] Replying to KontrollKast: "); Serial.println(out);
            rs485Send(out);
        }
    }

    // ── rs485_ping ────────────────────────────────────────────────────────────
    else if (strcmp(type, "rs485_ping") == 0) {
        Serial.println("  → RS485 ping received, replying");
        StaticJsonDocument<100> reply;
        reply["type"] = "rs485_pong";
        reply["mac"]  = WiFi.macAddress().c_str();
        String out;
        serializeJson(reply, out);
        rs485Send(out);
